<?php

// XOR function
function x($t, $k) {
    $c = strlen($k);
    $l = strlen($t);
    $o = "";
    for ($i = 0; $i < $l;) {
        for ($j = 0; ($j < $c && $i < $l); $j++, $i++) {
            $o .= $t[$i] ^ $k[$j];
        }
    }
    return $o;
}

// Main function
function deobfuscate($input_string) {
    $k = "80e32263";
    $kh = "6f8af44abea0";
    $kf = "351039f4a7b5";

    if (preg_match("/$kh(.+)$kf/", $input_string, $m) === 1) {
        $decoded_payload = base64_decode($m[1]);
        $xor_result = x($decoded_payload, $k);
        $decompressed = @gzuncompress($xor_result);

        if ($decompressed === false) {
            return "Error: Unable to decompress the data.";
        }
        return $decompressed;
    }
    return "Error: Invalid input string format.";
}

$input_string = "351039f4a7b5";
$result = deobfuscate($input_string);

echo $result;

?>
